...
% On suppose que $\sum_{k=1}^n \lambda_k \neq -1$ et on se donne
%   $\mu_1,\ldots,m_u\in\K$ tels que $\mu_1 y_1+\cdots+\mu_n y_n
%   ,
%   alors la famille $(y_1,\ldots,y_n)$ est libre.
% \question Réciproquement, montrer que si la famille $(y_1,\ldots,y_n)$ est libre, alors
%  .
\end{questions}
...
\magsubsection{Famille Génératrice}
...
\magsubsection{Cas des familles infinies}

\exercice{nom={Polynômes}}
Pour tout $n\in\N$, on pose
\[P_n\defeq\prod_{k=0}^{n-1} (X-k).\]
Montrer que la famille $(P_n)_{n\in\N}$ est libre.

\exercice{nom={Famille des fonction polynôme-exponentielle}}
...
\magsubsection{Espace vectoriel de dimension finie}
...
\magsubsection{Dimension d'un espace vectoriel}
\magsubsection{Existence et unicité en dimension finie}
...
\magsubsection{Dimension d'un sous-espace vectoriel}

\magsubsection{Notion de rang}
...
\magsubsection{Somme de deux sous-espaces vectoriels}
...
\magsubsection{Produit de deux sous-espaces vectoriels, $\mathcal{L}(E,F)$}
\magsubsection{Théorème du rang}
...
\exercice{nom={Noyaux et images itérés}}
...
\exercice{nom={Intersection d'hyperplans}}
...
\exercice{nom={Hyperplan de $\polyK$}}
